...
void draw()
{
  ...
  // y-axis label
  rotate(-PI / 2);
  text("VALUE0", width * -0.5, height * 0.023);
  rotate(PI / 2);
  ...
  int y_counter = 0;
  // change array value when needed
  ...
  for (TableRow row : table.rows())
  {
    ...
    // draw bar
    rect(tick_x_axis_x_val - (0.25 * x_axis_division), y_axis_y1_val - (value0 * height * 0.09), x_axis_division / 2, (value0 * height * 0.09));
    fill(0);
    // USEFUL FOR LATER PROJECTS
    // draw corresponding tick
    //line(tick_y_axis_x1_val, y_axis_y1_val - (value0 * height * 0.09), tick_y_axis_x2_val, y_axis_y1_val - (value0 * height * 0.09));
    // draw corresponding value
    //text(value0, tick_y_axis_x_num, (y_axis_y1_val - (value0 * height * 0.09) + (height * 0.005)));
    
    // increment x-axis ticks
    tick_x_axis_x_val += x_axis_division;
    
    // create array to store y-axis values
    y_vals[y_counter] = value0;
    ...
    {
      float max_y_value = round(max(y_vals));
