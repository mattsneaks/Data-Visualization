...
{ 
  ...
  float[] xStorage = new float[table.getRowCount()];
  float[] yStorage = new float[table.getRowCount()];
    ...
    for (int row = 0; row < table.getRowCount(); row++)
    {
      ...
      { 
        ...
        for (int i = 0; i < table.getRowCount(); i++)
        {
          ...
          if (i > 0)
            line(xStorage[i - 1], yStorage[i - 1], xStorage[i], yStorage[i]);
